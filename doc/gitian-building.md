...
*Setup instructions for a Gitian build of Mitocoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Mitocoin
...
- [Building Mitocoin Core](#building-mitocoin-core)
...
Clone the git repositories for mitocoin and Gitian.
...
in the mitocoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Mitocoin Core
----------------

To build Mitocoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the mitocoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mitocoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mitocoin-project/mitocoin
...
For example:
```bash
URL=https://github.com/thrasher-/mitocoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mitocoin=${COMMIT} --url mitocoin=${URL} ../mitocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mitocoin=${COMMIT} --url mitocoin=${URL} ../mitocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mitocoin=${COMMIT} --url mitocoin=${URL} ../mitocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mitocoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mitocoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mitocoin
SIGPATH=/some/root/path/mitocoin-detached-sigs

./bin/gbuild --url mitocoin=${BTCPATH},signature=${SIGPATH} ../mitocoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.mmc` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mitocoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mitocoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mitocoin-osx-build.assert
```
...
[gitian.sigs.mmc](https://github.com/mitocoin-project/gitian.sigs.mmc/) repository, or if that's not possible create a pull
